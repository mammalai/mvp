...
export default function NavItem({ item, level, isParents = false, setSelectedID }) {
  ...
  const iconSelectedColor = theme.palette.m3.onSecondaryContainer;
  ...
  const itemIcon = item.icon ? (
    <Icon
      style={{
        ...
        color: isSelected ? iconSelectedColor : theme.palette.m3.onSurfaceVariant,
        fontSize: 16,
    ...
    <>
      <Box sx={{ position: 'relative' }}>
        <ListItemButton
          ...
          sx={(theme) => ({
            borderRadius: 28,
            marginLeft: drawerOpen ? 1 : 0,
            marginRight: drawerOpen ? 1 : 0,
            zIndex: 1201,
            pl: drawerOpen ? `${level * 28}px` : 1.5,
            py: !drawerOpen && level === 1 ? 1.25 : 1,
            ...(drawerOpen && {
              backgroundColor: 'transparent',
              '&:hover': { backgroundColor: alpha(theme.palette.m3.onSurface, 0.08) },
              '&.Mui-selected': {
                backgroundColor: theme.palette.m3.secondaryContainer,
                '&:hover': { backgroundColor: blend(theme.palette.m3.secondaryContainer, theme.palette.m3.onSecondaryContainer, 0.08) }
              }
            }),
            ...(!drawerOpen && {
